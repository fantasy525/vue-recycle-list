...
let updateTextTime = performance.now();
const setText = (node: HTMLElement, text: string) => {
  if (performance.now() - updateTextTime < 16.7) {
    requestAnimationFrame(() => {});
    node.textContent = text;
  }
};
const updateNode = (targetPos: (typeof viewPos)[0]) => {
...
const onScroll = (offset: number) => {
  ...
  if (
    ...
    index + count.value <= dataList.list.length
  ) {
    offsetIndex.value = index;
    const targetInfo = recyleNode();
    layoutNode();
